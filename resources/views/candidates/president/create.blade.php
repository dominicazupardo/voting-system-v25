<x-app-layout>
    <div class="flex h-screen">
        ...
        <div class="flex-grow bg-gradient-to-br bg-white text-blue-900 p-10">
            <h1 class="text-3xl font-bold text-blue-900 mb-6">List of Candidates for President</h1>
                ...
                <table class="w-full border-collapse">
                    ...
                    <tbody id="results-body">
                        ...
                        <tr class="border-b bg-white text-blue-900">
                            <td class="px-6 py-4">{{ sprintf("%s %s. %s", $president->firstname, substr($president->middlename, 0, 1), $president->lastname) }}</td>
                            <td class="px-6 py-4">{{ sprintf("PR-%s-00%s", date('Y'), $president->id) }}</td>
                            <td class="px-6 py-4">{{ $president->partylist_name }}</td>
                            <td class="px-6 py-4 text-left">
                                <form action="{{ route('presidents.destroy', $president->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
